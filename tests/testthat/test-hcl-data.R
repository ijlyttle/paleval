hex <- c("#000000", "#FFFFFF")
bloom_int <- .hcl(hex)
bloom <- pev_data_hcl(hex)

test_that("internal implemenation works", {

  expect_is(bloom_int, "data.frame")
  expect_named(bloom_int, c("x", "hue", "chroma", "luminance", "is_rgb_limit"))
  ...
  expect_identical(bloom_int$is_rgb_limit, c(TRUE, TRUE))
...
test_that("hex implementation works", {
  ...
  expect_named(
    bloom,
    c("cvd", "x", "hex", "hue", "chroma", "luminance", "is_rgb_limit")
  )
  ...
  expect_true(all(bloom$is_rgb_limit))
...
test_that("bloom implementations are equivalent", {
  ...
  bloom_ref <- pev_data_hcl(hex_ref)

  # continuous
  expect_identical(
    name_ref %>% pev_fcont() %>% pev_data_hcl(n = n_ref),
    bloom_ref
  )
  # discrete bounded
  expect_identical(hex_ref %>% pev_fdisc() %>% pev_data_hcl(), bloom_ref)
  # discrete unbounded
  expect_identical(
    name_ref %>% pev_fcont() %>% pev_fdisc() %>% pev_data_hcl(n = n_ref),
    bloom_ref
  )
  # name of colorspace palette
  expect_identical(name_ref %>% pev_data_hcl(n = n_ref), bloom_ref)

  # cvd = none
  expect_identical(
    name_ref %>% pev_data_hcl(n = n_ref, include_cvd = FALSE),
...
test_that("bloom errors", {
  expect_error(pev_data_hcl(1), "No method")
})
